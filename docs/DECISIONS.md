...
| Story templates | five bound stories in `shared/templates/` (love-letter, leaf-umbrella, sunflower, summer-festival, snow-rabbit), 19 to 25 pages each, 22 to 65 SVG pieces, all under the 48,000-character warning, checked with `pnpm ehon-check` and the JVM template tests, rendered through the web painter; published to R2 behind the assets Worker (`templates/`), fetched by both clients at runtime (#60) |
