...
Web studio and backend (decisions 29–51, 57–58): `WEB_BACKEND_PLAN.md` is the implementation plan.
...
| 58 | **The web is for signed-in grown-ups.** A landing page stands in front of every page except guest links; the web has no anonymous stage; sign-in is Apple, Google (brand marks inline) or a **QR hand-off from the phone app**: the browser opens a login request (`qrLoginStart`), the signed-in app scans the code or types its six letters and approves (`qrLoginApprove`), the browser claims one custom token carrying `handoff: 'app'` (`qrLoginClaim`), which the rules and callables accept beside the providers. Every CTA carries an icon | Owner correction, 2026-09-13: the studio was usable without an account, which fits neither a product with sync at its centre nor a children's app whose grown-up side must be deliberate. The phone stays anonymous-first (#15) because a child is holding it; the web is where the parent already has an account, so the phone can vouch for the browser without typing a password. The token is minted only at claim time, so no bearer credential ever rests in Firestore; the QR code carries only the request id and a camera scan lands on a notice page rather than a second sign-in. The no-cloud checkout keeps no gate because it has no accounts: it is the local harness the browser tests run against |
| 59 | **The template shelf is five bound stories, each a whole picture book: the author's love-letter storyboard and four originals (はっぱの かさ, おおきく なあれ, はじめての なつまつり, ゆきうさぎの よる), every one a cover, a title page, eight spreads picture-left words-right and a colophon, with every character a movable piece and every line an editable text item. The nine three-page sketches are retired** | Owner direction, 2026-09-13: the love-letter book set the bar ("illustrations and stories matter"), and three-page mood pieces beside it looked like placeholders. Fewer, complete stories give a family something to read before they change it, and each story's README keeps the spread table and the off-page narration so an illustrator or a model can extend it. Composition rules learned on the way: anchor pieces to points on other drawings (a frog's fist, a bear's head top) rather than to guessed page coordinates; a sad character sits hugging its knees, front view; words drawn inside a picture on a dark sky take `colorIndex` 1 because ink vanishes on navy; keep each book under the 48,000-character warning so it syncs with room to spare |
| 60 | **Story templates live on the backend, not in the apps.** Gradle assembles `shared/templates/*` into `.ehon` documents plus a content-addressed `index.json`; `make templates-upload` publishes them under the assets Worker's `templates/` prefix (index cached five minutes, documents immutable); the web (`src/templates.ts`) and the phone (`TemplateCatalog`) fetch the index at launch, then each document on demand, and keep what they fetched. The four prompt-only starters are retired; the blank book is the only procedural template left, and the browser tests start from the blank page | Owner direction, 2026-09-13: the template inventory is the five stories, and templates must be updatable without shipping clients. A published document is a file the checker already validates, so the pipeline is the existing assets Worker plus one upload tool, no new function; content-addressed keys make caching safe and an upload atomic (documents first, index last). A checkout without cloud reads the same layout from the dev server, so the harness and the browser tests stay offline. Bundling nothing means a first launch offline offers only the blank book; a phone that fetched once keeps its copy |
| 61 | **Development builds read the assembled templates locally.** The Vite dev server defaults to its own `/templates` (the assembled build) whatever `.env.local` names, and a Debug iOS build bundles `shared/build/templates` through a post-build phase that `TemplateCatalog` reads first; `VITE_TEMPLATES_URL` and `EHON_REMOTE_TEMPLATES` opt back into the published index. Release builds bundle nothing and fetch as #60 says | Owner direction, 2026-09-14: testing should need no call to the assets Worker. Gradle already assembles the stories on every Debug build of the phone and on `make web`, so the local copy is always the current sources, a change to a story shows up on the next build without an upload, and the simulator and a fresh checkout list the stories offline and without a cloud configuration; the published pipeline stays the only path a release build knows |
| 62 | **The phone starts a book the way the web does: one blank white page from the shelf, and the templates screen lists the stories only.** `Templates.blankBook` in the core builds that page for both clients (the web's `EhonCodec.blankBook` delegates to it); the shelf's first two tiles are まっしろな ページから and テンプレートから, the つくる tab opens the blank page, and the four-page blank template with its shape picker (#17) leaves the screen but stays as baseline and test scaffolding | Owner direction, 2026-09-14: align the app with the web (#52, #58) and let the backend be the shared source for signed-in users. One definition of a new book means a book begun on either device is the same document on the other, the templates list carries only the bound stories the backend publishes, and a shape choice for a blank page was the one thing the web never offered |
| 63 | **The web reader turns pages with the phone's drag-driven fold (decision #6), and on both the leaf is a soft sheet: twelve hinged strips that bow off the edge the finger holds and flatten as they land, shaded by their own curve.** Web: CSS 3D with one perspective, faces painted once per turn, keyboard, edge taps and dots ride the same animation, printing unchanged. Phone: `LeafFold`, an `Animatable` modifier projecting each strip through one camera (`projectionEffect`), faces rendered once with `ImageRenderer` | Owner request, 2026-09-14: the web slid spreads sideways while the phone folded them, and a rigid leaf reads as card, not paper. Curl was low on the book-feel list (#6) but a bowed sheet costs no shaders: the same rotationY math, twelve times, with the accumulated angle |
...
| Story templates | five bound stories in `shared/templates/` (love-letter, leaf-umbrella, sunflower, summer-festival, snow-rabbit), 19 to 25 pages each, 22 to 65 SVG pieces, all under the 48,000-character warning, checked with `pnpm ehon-check` and the JVM template tests, rendered through the web painter; published to R2 behind the assets Worker (`templates/`), fetched by both clients at runtime (#60), development builds read the assembled copy (#61) |
...
| Web | landing page and sign-in gate (Apple, Google, QR hand-off from the phone), shelf, composer (`/app/:id`: frames desk with pan/zoom and scratch items, autosave, lease, remote replacement, illustration library, PNG upload, face picker) and reader (`/read/:id`, `/g/:token`, the phone's leaf fold as a soft sheet) over the Kotlin/JS core; icons on every CTA; sliced woff2 fonts — 32 harness/desk/reader/painter tests, 6 emulator scenarios (`make web-sync-test`), Lighthouse byte budgets (`make web-budget`), GitHub Actions workflow |
| Backend | Firestore rules (incl. `workspaces`), asset presign/finalize/signed read, guest links, QR login (`qrLoginStart/Approve/Claim`), cleanup, budget guard, `localBlob` — 22 emulator tests (`make rules-test`); dev project `petapeta-dev` provisioned 2026-09-13: Tokyo Firestore with rules and indexes, twelve functions, R2 bucket behind the assets Worker, the web on Cloudflare Pages; App Check registration follows |
...
- **Web studio and backend**: Phases 0–2 of `WEB_BACKEND_PLAN.md` are implemented locally and
  ...
  Lighthouse budgets in CI; provisioning began 2026-09-13 with the dev project `petapeta-dev`
  (Tokyo Firestore, rules, indexes, the functions on Blaze, Apple and Google sign-in, R2 behind
  the assets Worker, the web on Pages behind the landing page); App Check registration and budget
  alerts follow, then Phase 3 (Klecks spike, AI, Stripe). The phone's QR approval needs a device
  smoke test against the dev project; Universal Links for the `/login/<id>` QR payload wait on an
  Apple team id.
