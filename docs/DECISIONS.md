...
| Web | landing page and sign-in gate (Apple, Google, QR hand-off from the phone), shelf, composer (`/app/:id`: frames desk with pan/zoom and scratch items, autosave, lease, remote replacement, illustration library, PNG upload, face picker) and reader (`/read/:id`, `/g/:token`) over the Kotlin/JS core; icons on every CTA; sliced woff2 fonts — 30 harness/desk/painter tests, 6 emulator scenarios (`make web-sync-test`), Lighthouse byte budgets (`make web-budget`), GitHub Actions workflow |
